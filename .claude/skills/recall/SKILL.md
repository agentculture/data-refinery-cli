...
description: >
  ...
  defaults queries to this agent's PERSONAL, PRIVATE scope (`--scope data-refinery-cli
...
- `--scope NAME` / `--visibility public|private` — scope isolation (no private
  ...
  (`--scope data-refinery-cli --visibility private`, suffix read from `culture.yaml`),
