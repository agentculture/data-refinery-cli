...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope data-refinery-cli
...
scope:** the default personal scope is **private** (`--scope data-refinery-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `data-refinery-cli`). Private records are served only to a recall in the same
