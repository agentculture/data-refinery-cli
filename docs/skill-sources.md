...
data-refinery-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. data-refinery-cli
...
#   - consumer-identifying prose: `guildmaster` → `data-refinery-cli` (NOT
...
If a re-sync would lose a data-refinery-cli adaptation, lift the change
...
#     → `which data-refinery-cli vendors from guildmaster`
