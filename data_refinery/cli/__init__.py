"""Unified CLI entry point for data-refinery-cli.

The agent-first global verbs (``whoami``, ``learn``, ``explain``, ``overview``,
``doctor``) are registered here under :mod:`data_refinery.cli._commands`,
alongside the ``cli`` noun group. Future noun groups register via their own
``register()`` functions following the same pattern.

Error propagation contract
--------------------------
Every handler raises :class:`data_refinery.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`data_refinery.cli._output`. Unknown exceptions are wrapped into a
...
from data_refinery.cli._output import emit_error

_ISSUES_URL = "https://github.com/agentculture/data-refinery-cli/issues"
...
def _build_parser() -> argparse.ArgumentParser:
    from data_refinery.cli._commands import cli as _cli_group
    from data_refinery.cli._commands import doctor as _doctor_cmd
